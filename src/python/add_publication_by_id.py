...
def wrangle_fetched_content(parsed, paper_json):
    ...
    paper_json['names'] = ", ".join(author_names).replace("\n", " ")
    ...
    if 'ArXiv' in paper_json['externalIds']:
        paper_json['link'] = f"https://arxiv.org/abs/{paper_json['externalIds']['ArXiv']}"
        paper_json['shorthand'] = paper_json['externalIds']['ArXiv']

    lab_members_names = {parsed['name'] for parsed in lab_members.values()}
    lab_members_ids = {parsed['semantic_scholar_id'] for parsed in lab_members.values() if 'semantic_scholar_id' in parsed}
    ...
    del paper_json['externalIds'], paper_json['paperId'], paper_json['url'], paper_json['authors']
